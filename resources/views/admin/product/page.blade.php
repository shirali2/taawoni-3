    ...
    <div class="col-12 mb-3">
      <div  class="card-box table-responsive">
        <div class="m-t-0 m-b-30">
          <h4 class="header-title d-inline-block ">فهرست اشتراک ها</h4>
          <a href="/admin/product/add" class="btn btn-success btn-trans waves-effect w-md waves-success m-b-5 float-right">افزودن اشتراک</a>
        </div>
        ...
        <div id="datatable-buttons_wrapper" class="dataTables_wrapper  dt-bootstrap4 p-0 m-0 no-footer">
          ...
          <table id="datatable-buttons" class="table demo datatable-buttons table-striped table-bordered p-0 m-0 dataTable no-footer" role="grid" aria-describedby="datatable-buttons_info" style="width: 100%;direction: rtl;" width="100%" cellspacing="0">
            <thead>
              <tr role="row">
                <th class="sorting_asc"  style="width: 15px" >شناسه</th>
                @php
                  foreach(array(
                    'نام',
                    'روز',
                    ...
                    echo "<th class='sorting'>$column_name</th>";
                  endforeach
                @endphp
                <th class="sorting" aria-label="Salary: activate to sort column ascending">اقدامات</th>
              </tr>
            </thead>
            ...
            <tbody>
              @foreach($products as $product)
                <tr role="row" class="odd">
                  ...
                  <td>{{$product["name"]}}</td>
                  <td>{{$product['number_days']}}</td>
                  <td>{{number_format($product['number_days'] * $product['amount_1_day'])}} تومان</td>
                  <td>
                    @if($product['name_grops'])
                      @foreach($product['name_grops'] as $name_grop)
                        <span class="float-left bg-info text-white m-1 p-1">{{$name_grop['name']}}</span>
                      @endforeach
                    @endif
                  </td>
                  <td>{{$product['status']}}</td>
                  <td class="d-block-s">
                    <a href="/admin/product/edit/{{$product["id"]}}" data-toggle="tooltip" data-placement="top" data-original-title="ویرایش" class="btn btn-warning"><i class="fa fa-pencil"></i></a>
                    <a href="/admin/product/status/change/{{$product["id"]}}" data-toggle="tooltip" data-placement="top" data-original-title="تغییر وضعیت" @if($product['status'] == 'غیرفعال') class="btn btn-danger" @else class="btn btn-success" @endif><i @if($product['status'] == 'غیرفعال') class="fa fa-lock" @else class="fa fa-unlock" @endif ></i></a>
