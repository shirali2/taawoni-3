...
@section('container')
        ...
        <div class="row">
            <div class="col-12 mb-3">
                <div class="card-box ">
                    ...
                    <form class="form-horizontal form" method="post" action="/admin/menu/edit/{{ $menu['id'] }}"
                        ...
                        <div class="row">
                            ...
                            <div class="col-12">
                                <div class="form-group row">
                                    ...
                                    <div class="col-sm-8">
                                        <select id="grop_select2" onchange="grop_change(event)" required name="grop[]"
                                            class="form-control select2 select2-multiple select2-hidden-accessible"
                                            multiple="multiple" data-placeholder="انتخاب مجموعه" tabindex="-1"
                                            aria-hidden="true">
                                            >
                                            @foreach ($grops as $grop)
                                                    <option @if (in_array($grop['id'], $grops_selected)) selected="selected" @endif value="{{ $grop['id'] }}">{{ $grop['name'] }}
                                                    </option>
                                ...
                                <div class="form-group row">
                                    ...
                                    <div class="col-sm-8">
                                        <select name="gropUser[]"
                                            class="form-control select2 select2-multiple select2-hidden-accessible"
                                            multiple="multiple" data-placeholder="انتخاب گروه کاربری مجموعه" tabindex="-1"
                                            aria-hidden="true" >
                                            @foreach ($user_grops as $info_user_grop)
                                                <option
                                                    value="{{ $info_user_grop['id'] }}"@if (in_array($info_user_grop['id'], $user_grops_selected)) selected="selected" @endif>
                                                    {{ $info_user_grop['name'] }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                                ...
                                <div class="form-group row">
                                    ...
                                    <div class="col-sm-8">
                                        <input parsley-trigger="change" value="{{ $menu['name'] }}" required name="name"
                                        ...
                                        <select name="form" class="form-control select2">
                                            ...
                                            @foreach ($forms as $form)
                                                @if ($form['id'] == $menu['id_form'])
                                                    <option selected value="{{ $form['id'] }}">{{ $form['name'] }}
                                                    </option>
                                                @else
    ...
    <script>
        ...
        function grop_change(event){
   
   var grop = [];
   $.each($('#grop_select2').select2('data'), function(index, value) {
grop.push(value.id); // Use value.id to get the value of the selected option
});
   console.log(grop)
   if(grop.length>1){
       document.querySelector("body > div > div.container-fluid > div > div > div > div > form > div > div:nth-child(1) > div:nth-child(2) > div > span > span.selection > span").remove();


   }
   $("#type").find("div").remove();
   var token = $("input[name='_token']").val();
   $.ajax('/admin/menu/add/gropUsers', {
       type: 'post',
       async: false,
       data: {
           _token: token,
       ...
       success: function(data1) {
           ...
           $("[name='gropUser[]'] option").remove();

           $.each(data, function(index, value) {
               $("[name='gropUser[]']").append('<option value="' + value["id"] + '">' + value[
                   "name"] + '</option>')
   ...
   $("select[name='grop[]']").change(function() {
       ...
       $.ajax('/admin/menu/add/gropUsers', {
           ...
           success: function(data1) {
               ...
               $.each(data, function(index, value) {
                   $("[name='gropUser[]']").append('<option value="' + value["id"] + '">' +
                       value["name"] +'</option>')
