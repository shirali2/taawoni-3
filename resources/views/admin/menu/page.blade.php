    ...
    <div class="col-12 mb-3">
      <div  class="card-box table-responsive">
        <div class="m-t-0 m-b-30">
          ...
          <a href="/admin/menu/add" class="btn btn-success btn-trans waves-effect w-md waves-success m-b-5 float-right">افزودن منو</a>
        </div>
              ...
              @foreach($menus as $menu)
                <tr role="row" class="odd">
                  ...
                  <td style="text-align: right !important;">
                    @if($menu['grop'])
                      @foreach($menu['grop'] as $grop)
                        <span class="float-left bg-info text-white m-1 p-2">{{$grop['name']}}</span>
                      @endforeach
                    @endif
                  </td>
                  <td style="text-align: right !important;">
                  @if($menu['names_user_grops'] && count($menu['names_user_grops']) <= 2)
...
@endif
                  ...
                  <td style="display: none;" class="d-block-s">
                    <a href="/admin/menu/under/{{$menu["id"]}}" data-toggle="tooltip" data-placement="top" data-original-title="زیر منو" class="btn btn-primary"><i class="fa fa-eye"></i></a>
                    <a href="/admin/menu/edit/{{$menu["id"]}}" data-toggle="tooltip" data-placement="top" data-original-title="ویرایش" class="btn btn-warning"><i class="fa fa-pencil"></i></a>
