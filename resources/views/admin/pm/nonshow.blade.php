    ...
    <div class="col-12 mb-3">
      <div  class="card-box table-responsive">
        <div class="m-t-0 m-b-30">
          <h4 class="header-title d-inline-block ">فهرست کاربرانی که پیام را مشاهده نکرده‌اند</h4>
        ...
        <div id="datatable-buttons_wrapper" class="dataTables_wrapper  dt-bootstrap4 p-0 m-0 no-footer">
          ...
          <table id="datatable-buttons" class="table demo datatable-buttons table-striped table-bordered p-0 m-0 dataTable no-footer" role="grid" aria-describedby="datatable-buttons_info" style="width: 100%;direction: rtl;" width="100%" cellspacing="0">
            ...
            <tbody>
              @foreach($un_Showpm as $pm_unshow)
                <tr role="row" class="odd">
                  <td style="text-align: right !important;" class="sorting_1">{{$pm_unshow["id"]}}</td>
                  <td style="text-align: right !important;">{{$pm_unshow["name"]}}</td>
                  <td style="text-align: right !important;">{{$pm_unshow["mobile"]}}</td>
                  <td style="text-align: right !important;">{{$pm_unshow["id"]}}</td>
                  <td style="text-align: right !important;">{{$pm_unshow["hash"]}}</td>
                  <td style="display: none;" class="d-block-s">
                    @if(\App\Http\Controllers\managerController::managerAccessLevel(10))
                      <button class="btn btn-danger btn-delete" id="{{$pm_unshow['id']}}" name="{{$pm_unshow['name']}} {{$pm_unshow['name2']}}" data-toggle="tooltip" data-placement="top" data-original-title="حذف" ><i class="fa fa-times"></i></button>
                    @endif
                  </td>
                </tr>
              @endforeach
