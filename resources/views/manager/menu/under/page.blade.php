        ...
        <div class="col-12 mb-3">
            <div  class="card-box table-responsive">
                <div class="m-t-0 m-b-30">
                    ...
                    <h4 class="header-title d-inline-block ">فهرست زیر منو های منو {{$menu["name"]}}</h4>
                    @if(\App\Http\Controllers\managerController::managerAccessLevel(30))
                    <a href="/manager/menu/under/add/{{$menu["id"]}}" class="btn btn-success btn-trans waves-effect w-md waves-success m-b-5 float-right">افزودن زیرمنو</a>
                     @endif
                </div>
                <div id="datatable-buttons_wrapper" class="dataTables_wrapper  dt-bootstrap4 p-0 m-0 no-footer">
                    ...
                    <table id="datatable-buttons" class="table demo datatable-buttons table-striped table-bordered p-0 m-0 dataTable no-footer" role="grid" aria-describedby="datatable-buttons_info" style="width: 100%;direction: rtl;" width="100%" cellspacing="0">
                                ...
                                <tr role="row">
                                    ...
                                    <th class="sorting"  style="text-align: right !important;" >نام زیر منو</th>
                                ...
                                @foreach($unders as $under)
                                    ...
                                    <tr role="row" class="odd">
                                        <td style="text-align: right !important;" class="sorting_1">{{$under["id"]}}</td>
                                        <td style="text-align: right !important;">{{$under["name"]}}</td>

                                        @if(\App\Http\Controllers\managerController::managerAccessLevel(32))
                                        <td style="display: none;" class="d-block-s">

                                            <a href="/manager/menu/under/edit/{{$under["id"]}}" data-toggle="tooltip" data-placement="top" data-original-title="ویرایش" class="btn btn-warning"><i class="fa fa-pencil"></i></a>
                                            <button class="btn btn-danger btn-delete" id="{{$under["id"]}}" name="{{$under["name"]}}" data-toggle="tooltip" data-placement="top" data-original-title="حذف" ><i class="fa fa-times"></i></button>
                                        </td>
                                        @endif
                                    </tr>
                                    @endforeach
