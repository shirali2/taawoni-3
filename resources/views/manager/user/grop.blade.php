...
@section('container')
        ...
        <div class="col-12 mb-3">
            <div class="card-box table-responsive">
                <div class="m-t-0 m-b-30">
                    ...
                    <h4 class="header-title d-inline-block ">فهرست لیست مجموعه کاربر {{ $user['name'] }}
                        {{ $user['name2'] }}</h4>
                    @if (\App\Http\Controllers\managerController::managerAccessLevel(47))
                        <a href="/manager/user/grop/add/{{ $user['id'] }}"
                ...
                <div id="datatable-buttons_wrapper" class="dataTables_wrapper  dt-bootstrap4 p-0 m-0 no-footer">
                    @if (old('message'))
                        <p class="alert alert-danger">{{ old('message') }}</p>
                    @endif
                    ...
                    <table id="datatable-buttons"
                        ...
                        <thead>
                            <tr role="row">
                                ...
                                <th class="sorting" style="text-align: right !important;">تاریخ عضویت</th>
                                @if (\App\Http\Controllers\managerController::managerAccessLevel(49))
                        ...
                        <tbody>
                            @foreach ($user_grops as $user_grop)
                                <tr role="row" class="odd">
                                    <td style="text-align: right !important;" class="sorting_1">{{ $user_grop['id'] }}</td>
                                    <td style="text-align: right !important;">{{ $user_grop['grop']['name'] }}</td>
                                    <td style="text-align: right !important;">{{ $user_grop['usergrop']['name'] }}</td>
                                    <td style="text-align: right !important;">{{ $user_grop['am'] }}</td>

                                    @if (\App\Http\Controllers\managerController::managerAccessLevel(49))
                                        <td style="display: none;" class="d-block-s">
                                            <button class="btn btn-danger btn-delete" id="{{ $user_grop['id'] }}"
                                                name="{{ $user_grop['usergrop']['name'] }}" data-toggle="tooltip"
                                                data-placement="top" data-original-title="حذف"><i
    ...
    <script>
        $(".btn-delete").click(function() {
            ...
            Swal.fire({
                html: '<div class="mt-3 font-sans"><img src="/img/clipart2619611.png" style="width: 150px;"><div class="mt-4 pt-2 fs-15 mx-5"><h4 class="font-sans">مطمئن هستید ؟  </h4><p style="direction: rtl;" class="text-muted mx-4 mb-0">ایا از حذف گروه کاربری   ' +
                    email +
                    ' مطمئن هستید ؟</p>           <form class="d-inline-block" action="/manager/user/grop/delete/' +
                    id +
