        ...
        <div class="col-12 mb-3">
            <div  class="card-box table-responsive">
                <div class="m-t-0 m-b-30">
                    ...
                    <h4 class="header-title d-inline-block ">فهرست مدیران سطح دسترسی {{$usergrop["name"]}} در مجوعه {{$grop["name"]}}</h4>
                    <a href="/manager/grop/access/{{$grop["id"]}}"
                ...
                <div id="datatable-buttons_wrapper" class="dataTables_wrapper  dt-bootstrap4 p-0 m-0 no-footer">
                    ...
                    <table id="datatable-buttons" class="table demo datatable-buttons table-striped table-bordered p-0 m-0 dataTable no-footer" role="grid" aria-describedby="datatable-buttons_info" style="width: 100%;direction: rtl;" width="100%" cellspacing="0">
                                ...
                                @foreach($managerAccess as $item)
                                    <tr role="row" class="odd">
                                        <td style="text-align: right !important;" class="sorting_1">{{$item['user']["id"]}}</td>
                                        <td style="text-align: right !important;">{{$item["user"]["name"]}}</td>
                                        <td style="display: none;" class="d-block-s">
                                            {{--<a href="/manager/grop/manager/edit/{{$grop["id"]}}" data-toggle="tooltip" data-placement="top" data-original-title="ویرایش" class="btn btn-warning"><i class="fa fa-pencil"></i></a>--}}
                                            <button class="btn btn-danger btn-delete" idGropUser="{{$usergrop["id"]}}" id="{{$item["id"]}}" name="{{$item["user"]["name"]}}" data-toggle="tooltip" data-placement="top" data-original-title="حذف" ><i class="fa fa-times"></i></button>
                                        </td>
