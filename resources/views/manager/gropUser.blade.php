        ...
        <div class="col-12 mb-3">
            <div  class="card-box table-responsive">
                <div class="m-t-0 m-b-30">
                    ...
                    <h4 class="header-title d-inline-block ">فهرست گروه کاربران مجموعه {{$grop["name"]}}</h4>
                  @if(\App\Http\Controllers\managerController::managerAccessLevel(59))
                    <a href="/manager/grop/user/add/{{$grop["id"]}}" class="btn btn-success btn-trans waves-effect w-md waves-success m-b-5 float-right">افزودن گروه کاربری جدید</a>
                   @endif
                 </div>
                    ...
                    <table id="datatable-buttons" class="table demo datatable-buttons table-striped table-bordered p-0 m-0 dataTable no-footer" role="grid" aria-describedby="datatable-buttons_info" style="width: 100%;direction: rtl;" width="100%" cellspacing="0">
                                ...
                                @foreach($usergrops as $usergrop)
                                    ...
                                    <tr role="row" class="odd">
                                        <td style="text-align: right !important;" class="sorting_1">{{$usergrop["id"]}}</td>
                                        <td style="text-align: right !important;">{{$usergrop["name"]}}</td>
                                        <td style="text-align: right !important;">{{$usergrop["text"]}}</td>

                                        <td style="display: none;" class="d-block-s">
                                            @if($usergrop["name"]!="کاربری عادی")
                                                @if(\App\Http\Controllers\managerController::managerAccessLevel(60))
                                                <a href="/manager/grop/user/edit/{{$usergrop["id"]}}" data-toggle="tooltip" data-placement="top" data-original-title="ویرایش" class="btn btn-warning"><i class="fa fa-pencil"></i></a>
                                                  @endif
                                                    @if(\App\Http\Controllers\managerController::managerAccessLevel(61))
                                                    <button class="btn btn-danger btn-delete" id="{{$usergrop["id"]}}" name="{{$usergrop["name"]}}" data-toggle="tooltip" data-placement="top" data-original-title="حذف" ><i class="fa fa-times"></i></button>
                                                      @endif
                                        @endif
                                        </td>
